...
<retrieved_memories>
  <memory type="runbook" confidence="0.92" id="rb_003">...</memory>
  <memory type="incident" confidence="0.75" id="inc_012">...</memory>
</retrieved_memories>
```
...
备忘录全文注入，不做截断。备忘录设计上就应该保持精简（[Architecture - 备忘录] 要求"过时内容及时清理"），如果超出预算说明备忘录本身需要整理。
...
#### Retrieved Memory Block — 检索记忆

检索到的记忆片段带来源标注，让模型能评估可信度：

```xml
<retrieved_memories>
  <memory type="runbook" confidence="0.92" id="rb_003" updated="2026-02-25">
  标题：Provider API 超时时的降级处理
  内容：
  1. 检查网络连通性（ping base_url）
  2. 确认 API Key 有效（healthCheck）
  3. 触发 Model Router 降级到下一个模型
  4. 通知用户模型已降级
  </memory>

  <memory type="incident" confidence="0.75" id="inc_012" updated="2026-02-20">
  标题：Anthropic API 间歇性超时事件
  内容：2月20日发现 claude-opus 调用间歇性超时，原因是 base_url 配置包含多余的路径段。
  修复：将 base_url 改为 https://api.anthropic.com（不带尾部斜杠）。
  </memory>
</retrieved_memories>
```

**无检索结果时不注入该标签**——空的 `<retrieved_memories></retrieved_memories>` 会误导模型以为检索系统出了问题。
