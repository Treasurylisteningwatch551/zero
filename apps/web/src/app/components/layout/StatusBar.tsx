...
export function StatusBar({ status, model, uptime, heartbeatAge, activeSessions }: StatusBarProps) {
  const statusConfig = {
    ...
    degraded: { label: 'Degraded', dot: 'bg-amber-400', text: 'text-amber-400', bg: 'bg-amber-400/5' },
    repairing: { label: 'Repairing', dot: 'bg-amber-400 pulse-active', text: 'text-amber-400', bg: 'bg-amber-400/5' },
    fused: { label: 'Fused', dot: 'bg-red-400 pulse-active', text: 'text-red-400', bg: 'bg-red-400/5' },
  }

  const cfg = statusConfig[status]
  const heartbeatColor =
    heartbeatAge > 50 ? 'text-red-400' : heartbeatAge > 30 ? 'text-amber-400' : 'text-[var(--color-text-disabled)]'

  return (
    <div className={`flex items-center gap-6 px-5 py-2 text-[11px] tracking-wide border-b border-[var(--color-border)] ${cfg.bg}`}>
