...
export class SessionDB {
  ...
  saveSession(data: SessionData, agentConfigJson?: string, systemPrompt?: string): void {
    this.db.run(
      `INSERT OR REPLACE INTO sessions
       (id, source, status, current_model, model_history_json, summary, tags_json, channel_id, agent_config_json, system_prompt, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        data.id,
        data.source,
   ...
   */
  getChannelMappings(): Array<{ id: string; source: SessionSource; channelId: string }> {
    const rows = this.db
      .query(`SELECT id, source, channel_id FROM sessions WHERE channel_id IS NOT NULL AND status IN ('active', 'idle')`)
      .all() as Array<{ id: string; source: string; channel_id: string }>
    return rows.map((r) => ({
      id: r.id,
